...
app.get("/api/loads", (req, res) => {
  ...
  if (req.query.to) {
    ...
    q +=
      " AND lower(l.to_city) LIKE ?";

    p.push(
      "%" +
      req.query.to.toLowerCase() +
      "%"
    );
        ...
        db.get(
          ...
          (err2, truck) => {
            ...
            if (err2 || !truck) {
              ...
              return res.status(400).json({
                error:
                  "Truck not found in your account"
              });
            ...
            db.get(
              ...
              (err3, existing) => {
                ...
                if (existing) {
                  ...
                  return res.status(400).json({
                    error:
                      "You already applied for this load"
                  });
...
app.delete(
  ...
  (req, res) => {
    ...
    db.serialize(() => {
      ...
      db.run(
        "DELETE FROM trucks WHERE user_id=?",
        [id]
      );
